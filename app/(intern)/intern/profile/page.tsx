...
// Intern profile page — same editable fields as Fellow
...
export default function InternProfilePage() {
  ...
  useEffect(() => { if (user) reset({ name: user.name, phone: user.phone }); }, [user, reset]);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-8">
      ...
      {/* Header Profile Card */}
      <div className="card p-6 bg-gradient-to-r from-emerald-900/5 via-slate-50 to-indigo-900/5 border-slate-200 text-center sm:text-left">
        <div className="flex flex-col sm:flex-row items-center sm:items-center gap-5">
          <div className="w-20 h-20 rounded-2xl bg-emerald-600 flex items-center justify-center text-white text-3xl font-bold shrink-0 shadow-md mx-auto sm:mx-0">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-3 max-w-md mx-auto sm:mx-0 text-left">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input {...register('name')} className={cn('w-full px-3 py-2 text-sm rounded-[var(--radius)] border bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500', errors.name ? 'border-rose-400' : 'border-slate-200')} />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 mb-1">Phone</label>
                    <input {...register('phone')} className="w-full px-3 py-2 text-sm rounded-[var(--radius)] border border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                  </div>
                </div>
                <div className="flex gap-2 pt-1">
                  <button type="submit" disabled={isSubmitting} className="flex items-center gap-1.5 px-4 py-2 rounded-[var(--radius-sm)] text-xs font-medium bg-emerald-600 text-white hover:bg-emerald-700 btn-press disabled:opacity-60">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 items-center">
                <div className="flex flex-col items-center sm:items-start">
                  <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2.5">
                    ...
                    <span className="badge bg-emerald-100 text-emerald-700 border-emerald-200">Intern (CMYIGGP)</span>
                  </div>
        ...
        <div className="lg:col-span-1">
          <div className="card p-6">
            ...
            <DetailList
              columns={1}
              rows={[
                { label: 'Program', value: 'CMYIGGP — Intern Program' },
                { label: 'State', value: 'Madhya Pradesh' },
                { label: 'District', value: user.block?.districtName },
                { label: 'Block', value: user.block?.name },
                { label: 'Gram Panchayat', value: user.gramPanchayat?.name },
                { label: 'Village', value: user.village?.name },
