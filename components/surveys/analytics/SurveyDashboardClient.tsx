...
interface SurveyDashboardClientProps {
  surveyId: string;
}

export default function SurveyDashboardClient({ surveyId }: SurveyDashboardClientProps) {
  ...
  return (
    <div className="space-y-6 pb-36 sm:pb-32 max-w-7xl mx-auto w-full min-w-0 max-w-full">
      ...
      {/* ── Sticky Question Navigator Strip (Freezes on top side with 2 mm gap when scrolling) ── */}
      <div className="sticky top-2 z-30 bg-white/95 backdrop-blur-md p-3 sm:p-3.5 rounded-2xl border border-slate-200/90 shadow-md ring-1 ring-slate-900/5 space-y-2.5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <div className="relative w-full sm:max-w-xs">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-1.5 text-xs rounded-xl border border-slate-200 bg-slate-50/60 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 text-slate-800 placeholder:text-slate-400 transition"
            />
          </div>
        </div>
