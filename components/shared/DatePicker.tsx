...
export default function DatePicker({
  ...
  return (
    ...
    >
      ...
      {/* Phone View: Centered Modal Pop-up with Backdrop */}
      {isOpen && mounted && createPortal(
        <div
          className="sm:hidden fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-150"
          onClick={() => {
            setIsOpen(false);
            setYearPickerOpen(false);
          }}
          ...
          >
            ...
            <div className="flex items-center justify-between pb-2.5 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-slate-900">{label || 'Select Date'}</span>
              </div>
              <button
                type="button"
                onClick={() => {
            ...
            <div className="pt-1">
              <button
                ...
                className="w-full py-2.5 rounded-xl bg-indigo-600 text-white text-xs font-bold hover:bg-indigo-700 active:scale-95 transition-all shadow-xs cursor-pointer"
              >
                Done
              </button>
            </div>
      ...
      {/* Desktop View: Popover Calendar Card */}
      {isOpen && (
        <div
          data-no-keyboard="true"
          className={cn(
            'hidden sm:block absolute z-50 mt-1.5 w-[320px] max-w-[calc(100vw-24px)] bg-white rounded-2xl border border-slate-200/90 shadow-[0_12px_36px_-6px_rgba(0,0,0,0.18),0_4px_12px_rgba(0,0,0,0.06)] p-3.5',
